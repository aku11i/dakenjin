...
describe("Sentence", () => {
  ...
  describe("input", () => {
    it("should input character to the current word", () => {
      const word1 = new Word({ label: "a", inputPatterns: ["a"] });
      word1.input("a");
      ...
      const sentence = new Sentence([word1, word2]);
      const result = sentence.input("b");
      expect(result).toBe(true);
      expect(sentence.isCompleted()).toBe(true);
    });

    it("should return false when input is invalid for current word", () => {
      const word1 = new Word({ label: "a", inputPatterns: ["a"] });
      word1.input("a");
      ...
      const sentence = new Sentence([word1, word2]);
      const result = sentence.input("c");
      expect(result).toBe(false);
      expect(sentence.isCompleted()).toBe(false);
    });

    it("should automatically advance to the next word", () => {
      const word1 = new Word({ label: "a", inputPatterns: ["a"] });
      const word2 = new Word({ label: "b", inputPatterns: ["b"] });

      const sentence = new Sentence([word1, word2]);

      sentence.input("a"); // First word completed
      expect(sentence.completedWords).toContain(word1);

      sentence.input("b"); // Second word should be current
      expect(sentence.completedWords).toContain(word2);
      expect(sentence.isCompleted()).toBe(true);
    });

    it("should throw error when there is no current word", () => {
      const word1 = new Word({ label: "a", inputPatterns: ["a"] });
      word1.input("a");
      ...
      const sentence = new Sentence([word1, word2]);
      expect(() => sentence.input("c")).toThrow(
        "No current word to input into",
      );
    });
    ...
    it("should throw error when trying to input more than one character", () => {
      ...
      expect(() => sentence.input("ab")).toThrow(
